const meetingsDetailsSection = document.getElementById(
...
if (currentMeeting) {
  const meetingDetailsTemplate = document.getElementById(
  ...
  const formattedDate = currentMeeting.date.toLocaleDateString(
    "en-US",
    options
  );
  ...
  if (currentMeeting.additionalInfo.length > 0) {
    ...
    const additionalInfoElement =
      additionalInfoTemplate.content.cloneNode(true);
    const infoContainer = additionalInfoElement.querySelector(
